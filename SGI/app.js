...
// ═══════════════════════════════════════════════════════
//  MODAL MANAGER  (con integración botón Atrás Android)
// ═══════════════════════════════════════════════════════
const MM = (() => {
    let _mdDown = false;
    const _onCerrar = {};

    // ── Flags de estado para el manejo del historial ──
    let _navegandoHaciaAtras = false;    // true mientras procesamos un popstate
    let _ignorarProximoPopstate = false; // true cuando hacemos history.back() nosotros mismos al cerrar
    let _enTransicion = false;           // true durante UI._nav (cierra+abre sin tocar el historial)

    // Guarda la longitud base del historial al iniciar la app.
    if (!sessionStorage.getItem('_SGI_historyBase')) {
        sessionStorage.setItem('_SGI_historyBase', String(window.history.length));
    }

    // ── Listener popstate (botón Atrás del sistema) ──
    ...
    function abrir(id, optsOrCb) {
        ...
        // Empujamos una entrada al historial para que el botón Atrás pueda cerrar
        // este modal, EXCEPTO cuando venimos de un popstate (ya retrocedió) o cuando
        // estamos en una transición padre→hijo (UI._nav): en ese caso el cierre del
        // padre ya retiró su entrada y el hijo empuja la propia en _enTransicion=false.
        if (!_navegandoHaciaAtras && !_enTransicion) {
            history.pushState({ sgiModal: id }, '');
        }
    ...
    function cerrar(id, cb) {
        ...
        // Retrocedemos en el historial para consumir la entrada que pushState dejó,
        // EXCEPTO si ya estamos retrocediendo (popstate) o en una transición padre→hijo.
        if (!_navegandoHaciaAtras && !_enTransicion) {
    ...
    // Marca el inicio/fin de una transición padre→hijo (UI._nav).
    // Durante la transición ni cerrar() hace history.back() ni abrir() hace pushState,
    // porque el resultado neto es "un modal reemplaza a otro": una sola entrada de historial.
    function iniciarTransicion() { _enTransicion = true; }
    function finalizarTransicion() { _enTransicion = false; }

    return { abrir, cerrar, cerrarTodos, cerrarTop, iniciarTransicion, finalizarTransicion };
})();
...
// ═══════════════════════════════════════════════════════
const UI = {
    // Helper padre→hijo: cierra `desde` y abre otro modal en su lugar.
    // El cierre del padre y la apertura del hijo cuentan como UN solo paso
    // en el historial (se usa _enTransicion para no hacer back/pushState doble).
    _nav(desde, fn) {
        MM.iniciarTransicion();
        MM.cerrar(desde);
        setTimeout(() => {
            fn();
            MM.finalizarTransicion();
        }, 150);
    },

    // Helper hijo→padre: cierra el hijo (hace history.back() de su entrada)
    // y luego abre el padre (hace pushState de su entrada).
    // El historial queda perfectamente balanceado.
    _back(desde, fn) {
        MM.cerrar(desde);          // consume la entrada del hijo con history.back()
        setTimeout(fn, 150);       // abre el padre, que hace su propio pushState
    },

    // Helper: cierra un modal hijo y vuelve a ajustes
    _cerrarAjustesHijo(id) {
        UI._back(id, () => UI.abrirAjustes());
    },
    ...
    cerrarNuevaHerramienta() {
        // hijo→padre: _back para que el historial quede balanceado
        UI._back('modal-herramienta-nuevo', () => {
...
function cerrarModalMaterial(sufijo) {
    if (_materialPadreId === 'ajustes') {
        ...
        UI._back(`modal-material-${sufijo}`, () => UI.abrirAjustes()); // hijo→padre balanceado
    } else {
